...
Use the existing Freenet container signing and publication path described by [application bundles](../appkit/bundles.md). Add the definition, action, delegate-interface and screen validation that plan requires. Preserve custom web entry points and relative assets. SDUI declares reader requirements. The application definition declares action and delegate requirements.

A participating product's workflow publishes, reads the archive back and obtains a separate signed contribution record for the bytes the node stored. Follow the [attribution sequence](../attribution/README.md#4-bundle-integration) for acceptance, certification and publication verification. Retain the archive read back, the contribution record and the signed envelope. After a timeout, check whether publication succeeded before submitting another version. A changed archive needs matching certification. Read back and verify the published archive, record whether the readback came from the publishing node or an independent node, and check retrieval from an independent node before advertising it. Enable paid use after the services confirm eligibility.
...
| 6. Publishing workflow | Validate, sign, publish, read back and certify when required | A new user publishes a custom web fixture, a reader-only fixture and a custom web fixture that embeds the reader. The certified digest is the stored digest, readback records the publication, and native links identify separately distributed apps |
