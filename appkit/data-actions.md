...
Each target has its own supported SDK path. Custom JavaScript/TypeScript applications use the existing TypeScript `freenet-stdlib`. Rust browser applications link Rust stdlib into their Wasm build. The web SDUI reader uses the proposed Rust-backed browser bindings. Native readers and custom native apps use the native library through Swift and Kotlin bindings. The [SDK paths table](../freenet-mobile/README.md#0-feasibility-and-existing-evidence) defines these paths and the feasibility work.

This plan owns AppKit's proposed declarative actions, data views and operation lifecycle. The SDK carries Freenet requests. The host checks permissions and coordinates work. Application delegates decode domain records, prepare updates and perform protected operations. Contracts validate and merge shared state. Custom applications can implement their own orchestration using their target's SDK and the same domain protocols.
...
## 2. Declarative actions and delegate interface
...
## 4. Values, storage and cached reads
...
## 5. Offline operations and cancellation
...
## 6. Attachments and device resources
...
## 8. Optional service integrations
...
## 9. Implementations and acceptance
