...
Web sessions have their own keys, sessions and grants. Ask for enrollment or authorization before handing access from the native app to a web session. Core's shell applies its existing capability-specific controls to the web target. The [host plan](../appkit/hosts.md#features-missing-in-freenet-for-appkit-to-work) lists the Core features web targets still need for AppKit grants. Payment runs in the app's own native screens, per the [payment plan](../payment/README.md#2-checkout-flow), and the in-app web view offers none.
